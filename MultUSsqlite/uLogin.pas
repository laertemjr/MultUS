...
begin
   ...
   dm.FDConnection1.DriverName := 'SQLite';

   if FileExists(ExtractFilePath(Application.ExeName) + 'config.ini') then
   begin
      iniconf := TIniFile.Create(ExtractFilePath(Application.ExeName) + 'config.ini');
      dm.FDConnection1.Params.Add('database=' + iniconf.ReadString('SQLite', 'Path', ''));
      iniconf.Free;
   end
   else
   begin
...
begin
   ...
   OpenDialog1.Title := 'Localize o Banco de Dados SQLite';

   if OpenDialog1.Execute then
   begin
      iniconf := TIniFile.Create(ExtractFilePath(Application.ExeName) + 'config.ini');
      iniconf.WriteString('SQLite', 'Path', OpenDialog1.FileName);
